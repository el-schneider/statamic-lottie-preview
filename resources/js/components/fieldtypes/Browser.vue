        ...
        >
            <div slot-scope="{ dragging }" class="min-h-screen">
                ...
                <data-list
                    ...
                    @selections-updated="
                        (ids) => $emit('selections-updated', ids)
                    "
                >
                        ...
                        >
                            <div class="relative w-full">
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        class="btn btn-sm rtl:mr-3 ltr:ml-3"
                                        @click="creatingFolder = true"
                                    >
                                        <svg-icon
                                            name="folder-add"
                                            class="h-4 w-4 rtl:ml-2 ltr:mr-2"
                                        />
                                    ...
                                    >
                                        <svg-icon
                                            name="upload"
                                            class="h-4 w-4 rtl:ml-2 ltr:mr-2 text-current"
                                        />
                                        <span>{{ __("Upload") }}</span>
                                    </button>
                            ...
                            <div class="overflow-x-auto overflow-y-hidden">
                                ...
                                >
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    <file-icon
                                                        extension="folder"
                                                        class="w-8 h-8 rtl:ml-2 ltr:mr-2 inline-block text-blue-400 group-hover:text-blue"
                                                    />
                                                    ..
                                                </a>
                                            </td>
                                    ...
                                    <template
                                        slot="cell-basename"
                                        slot-scope="{ row: asset, checkboxId }"
                                    >
                                        <div
                                            class="flex items-center w-fit-content group"
                                        >
                                            <asset-thumbnail
                                                :asset="asset"
                                                :square="true"
                                                class="w-8 h-8 rtl:ml-2 ltr:mr-2 cursor-pointer"
                                                @click.native.stop="
                                        ...
                                        <div class="asset-thumb-container">
                                            ...
                                            >
                                                <div class="asset-thumb">
                                                    <file-icon
                                                        extension="folder"
                                                        class="w-full h-full text-blue-400 hover:text-blue"
                                                    />
                                                </div>
                                            </button>
                                        </div>
                                        <div
                                            class="asset-meta flex items-center"
                                        >
                                            <div
                                                class="asset-filename text-center w-full px-2 py-1"
                                            >
                                                ..
                                            </div>
                                        </div>
...
import AssetEditor from "./Editor.vue";
import Breadcrumbs from "@components/assets/Browser/Breadcrumbs.vue";
import CreateFolder from "@components/assets/Browser/CreateFolder.vue";
import HasPagination from "@components/data-list/HasPagination";
import HasPreferences from "@components/data-list/HasPreferences";
import Uploader from "@components/assets/Uploader.vue";
import Uploads from "@components/assets/Uploads.vue";
import HasActions from "@components/data-list/HasActions";
...
export default {
    ...
    computed: {
        ...
        canEdit() {
            return (
                this.can("edit " + this.container.id + " assets") ||
        ...
        editedAssetBasename() {
            let asset = _.find(
                this.assets,
                (asset) => asset.id == this.editedAssetId
            );
    ...
    watch: {
        ...
        editedAssetId(editedAssetId) {
            let path = editedAssetId
                ? [this.path, this.editedAssetBasename]
                      .filter((value) => value != "/")
                      .join("/") + "/edit"
                : this.path;
    ...
    methods: {
        ...
        loadAssets() {
            ...
            const url = this.searchQuery
                ? cp_url(
                      `assets/browse/search/${this.container.id}/${
                          this.restrictFolderNavigation ? this.path : ""
                      }`
                  ).replace(/\/$/, "")
                : cp_url(
                      `assets/browse/folders/${this.container.id}/${
                          this.path || ""
                      }`
                  ).replace(/\/$/, "");
        ...
        selectRange(from, to) {
            for (var i = from; i <= to; i++) {
                ...
                if (
                    !this.selectedAssets.includes(asset) &&
                    !this.reachedSelectionLimit
                ) {
