<template>
    ...
    >
        <div
            class="asset-editor flex flex-col relative bg-gray-100 dark:bg-dark-800 h-full rounded"
        ...
        >
            ...
            <template v-if="!loading">
                ...
                <div
                    ...
                    class="flex justify-between w-full px-2 relative"
                >
                    <button
                    ...
                    >
                        <svg-icon
                            name="folder-image"
                            class="text-gray-700 dark:text-dark-175 h-5 w-5"
                        />
                        <span
                            class="rtl:mr-2 ltr:ml-2 text-sm text-gray-800 dark:text-dark-150 group-hover:text-blue dark:group-hover:text-dark-100"
                            >{{ asset.path }}</span
                        >
                        <svg-icon
                            name="micro/chevron-right"
                            class="text-gray-700 dark:text-dark-175 h-5 w-5 group-hover:text-blue dark:group-hover:text-dark-100 rtl:rotate-180"
                        />
                    </button>
                    <button
                        class="btn-close absolute top-2 rtl:left-2.5 ltr:right-2.5"
                        @click="close"
                ...
                <div
                    class="flex flex-1 flex-col md:flex-row md:justify-between grow overflow-scroll"
                >
                    <!-- Visual Area -->
                    <div
                        class="editor-preview bg-gray-800 dark:bg-dark-950 rtl:md:rounded-tl-md ltr:md:rounded-tr-md flex flex-col justify-between flex-1 min-h-[45vh] md:min-h-auto md:flex-auto md:grow w-full md:w-1/2 lg:w-2/3 shadow-[inset_0px_4px_3px_0px_black]"
                    >
                        <!-- Toolbar -->
                        <div
                            id="asset-editor-toolbar"
                            class="@container/toolbar flex items-center justify-center py-4 px-2 text-2xs text-white dark:text-dark-100 text-center space-x-1 sm:space-x-3 rtl:space-x-reverse"
                            v-if="isToolbarVisible"
                        >
                            <button
                                v-if="isImage && isFocalPointEditorEnabled"
                                type="button"
                                class="flex bg-gray-750 dark:bg-dark-400 hover:bg-gray-900 dark:hover:bg-dark-600 hover:text-yellow-light dark:hover:text-yellow-dark rounded items-center justify-center px-3 py-1.5"
                                @click.prevent="openFocalPointEditor"
                            >
                                <svg-icon name="focal-point" class="h-4" />
                                <span
                                    class="rtl:mr-2 ltr:ml-2 hidden @3xl/toolbar:inline-block"
                                    >{{ __("Focal Point") }}</span
                                >
                            </button>
                            ...
                            <button
                                ...
                                type="button"
                                class="flex bg-gray-750 dark:bg-dark-400 hover:bg-gray-900 dark:hover:bg-dark-600 hover:text-yellow-light dark:hover:text-yellow-dark rounded items-center px-3 py-1.5"
                            ...
                            <button
                                ...
                                class="flex bg-gray-750 dark:bg-dark-400 hover:bg-gray-900 dark:hover:bg-dark-600 hover:text-red-400 dark:hover:text-dark-red rounded items-center text-center px-3 py-1.5"
                            >
                        ...
                        >
                            <div class="image-wrapper">
                                ...
                                <!-- SVG -->
                                <div
                                    v-else-if="asset.isSvg"
                                    class="bg-checkerboard h-full w-full flex flex-col"
                                >
                                    <div
                                        class="flex border-b-2 border-gray-900"
                                    >
                                        <div
                                            class="flex-1 order-r p-4 border-gray-900 flex items-center justify-center"
                                        >
                                            <img
                                                :src="asset.url"
                                                class="asset-thumb w-4 h-4"
                                            />
                                        </div>
                                        <div
                                            class="flex-1 rtl:border-r ltr:border-l rtl:border-l ltr:border-r p-4 border-gray-900 flex items-center justify-center"
                                        >
                                            <img
                                                :src="asset.url"
                                                class="asset-thumb w-12 h-12"
                                            />
                                        </div>
                                        <div
                                            class="flex-1 rtl:border-r ltr:border-l p-4 border-gray-900 flex items-center justify-center"
                                        >
                                            <img
                                                :src="asset.url"
                                                class="asset-thumb w-24 h-24"
                                            />
                                        </div>
                                    </div>
                                    <div
                                        class="min-h-0 h-full p-4 flex items-center justify-center"
                                    >
                                        <img
                                            :src="asset.url"
                                            class="asset-thumb w-2/3 max-w-full max-h-full"
                                        />
                                    </div>
                                </div>
                        ...
                        <div
                            class="h-full"
                            v-else-if="
                                asset.isPreviewable && canUseGoogleDocsViewer
                            "
                        >
                            <iframe
                                class="h-full w-full"
                    ...
                    >
                        <div
                            class="w-full sm:p-4 md:pt-px md:w-1/3 md:grow h-1/2 md:h-full overflow-scroll"
                        ...
                        >
                            ...
                            <div
                                v-if="error"
                                class="bg-red-500 text-white p-4 shadow mb-4"
                                v-text="error"
                            />
                ...
                <div
                    class="bg-gray-200 dark:bg-dark-550 w-full border-t dark:border-dark-200 flex items-center justify-end py-3 px-4 rounded-b"
                >
                    <div
                        id="asset-meta-data"
                        class="flex-1 hidden sm:flex space-x-3 rtl:space-x-reverse py-1 h-full text-xs text-gray-800 dark:text-dark-150"
                    >
                        <div
                            class="flex items-center bg-gray-400 dark:bg-dark-600 rounded py-1 rtl:pr-2 ltr:pl-2 rtl:pl-3 ltr:pr-3"
                            v-if="isImage"
                        >
                            <svg-icon
                                name="image-picture"
                        ...
                        >
                            <svg-icon
                                ...
                                class="h-3 rtl:ml-2 ltr:mr-2"
                            />
                            <div class="" :title="asset.lastModified">
...
<script>
import EditorActions from "@components/assets/Editor/EditorActions.vue";
import FocalPointEditor from "@components/assets/Editor/FocalPointEditor.vue";
import PdfViewer from "@components/assets/Editor/PdfViewer.vue";
import PublishFields from "@components/publish/Fields.vue";
import HasHiddenFields from "@components/publish/HasHiddenFields";
    ...
    methods: {
        ...
        shouldClose() {
            if (this.$dirty.has(this.publishContainer)) {
                if (
                    !confirm(__("Are you sure? Unsaved changes will be lost."))
                ) {
                    return false;
                }
            }
